...
import data.cifar10 as cfr

cfr.download()
print(cfr.load_class_names())
train_img, train_cls, train_labels = cfr.load_test_data() #Sınıfların encode edilmiş halleri labelda
test_img, test_cls, test_labels = cfr.load_test_data()

#placeholderlar boş değişken tanımlayıp bırakamayacağımızdan tanınmlanmış
x = tf.placeholder(tf.float32, [None, 32, 32, 3])
y_true = tf.placeholder(tf.float32, [None, 10])
pkeep = tf.placeholder(tf.float32) #dropout oranı
phase=tf.placeholder(tf.bool)

#augmentation için fonksiyon
...
flattened = tf.reshape(conv3, [-1, 4 * 4 * 64]) #conv layerından çıkan 3 boyutlu matrisi vektör haline getirir.
...
y_pred_cls = tf.argmax(y, 1) #predictionlar

xent = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=y_true)#bulunan değer ile gerçek değer arasında ki fark kaybı verecektir.
loss = tf.reduce_mean(xent)

correct_prediction = tf.equal(y_pred_cls, tf.arg_max(y_true, 1)) #etiketlenen ile gerçekte olan aynı mı
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32)) #correct_predictionların ortalaması aslında

global_step=tf.Variable(0,trainable=False) #tensorflow variable'la tanımlanan şeyleri eğitilecek şeyler olarak algılar aksi belirtilmedikçe.Burada aksi belirtildi.
optimizer = tf.train.AdamOptimizer(5e-4).minimize(loss,global_step) #AdamOptimizer kullanarak kaybı en aza çekmeye çalışıyoruz.
...
#Train setinden batcleri random seçebilmek için
def random_batch():
...
def test_accuracy():
    ...
    correct = (test_cls == cls_pred) #Tahmin edilen değerle gerçek değerlerin karşılaştırılması
